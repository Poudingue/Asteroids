...
let asteroid_min_spawn_radius = 25.;;(*Taille min, en ratio de la taille de spawn max*)
...
let ship_density = 20.;; (*Pour calcul de la masse du vaisseau, qui a un impact sur la physique*)
...
let explosion_min_exposure = 0.9;;(*Détermine la luminosité max et min des explosions au spawn*)
...
let explosion_min_exposure_heritate = 20.;;(*Détermine la luminosité max et min des explosions au spawn*)
let explosion_max_exposure_heritate = 50.;;
...
let smoke_max_speed = 100.;;(*Vitesse random dans une direction random de la fumée*)
...
let rec render_light_trail radius pos velocity color =
  ...
  let (dx,dy) = multuple velocity ~-. (!game_speed *. ratio_rendu *. (max (shutter_speed /. framerate_render) (shutter_speed *.(!time_current_frame -. !time_last_frame)))) in
...
let render_objet ref_objet =
  ...
  set_color (rgb_of_hdr (intensify objet.hdr_color (0.5 *. objet.hdr_exposure)));
  ...
  fill_circle ((dither x) - width) ((dither y) + height)  (dither (ratio_rendu *. objet.radius));
  fill_circle ((dither x) - width) ((dither y) - height)  (dither (ratio_rendu *. objet.radius));
  (*Dessin du trait blanc, qui sert à montrer l'orientation d'un objet. Utile pour le ship*)
...
(*Rendu des explosions et de la fumée - non modulo*)
let render_smoke ref_objet =
  let objet = !ref_objet in
  let (x,y) = multuple objet.position ratio_rendu in
  (*Rendu de la vie de l'objet*)
  set_color (rgb_of_hdr (intensify objet.hdr_color objet.hdr_exposure));
...
let decay_smoke ref_smoke =
  ...
  if smoke.hdr_exposure > 0.01 then  smoke.hdr_exposure <- (exp_decay smoke.hdr_exposure smoke_half_life);
  ref smoke;;
...
let spawn_fire ref_ship = ref {
  objet = Smoke;
  radius = muzzle_ratio_radius *. !ref_ship.radius;
  mass = 0.;
  health = 0.;
  max_health = 0.;
...
let affiche_etat ref_etat =
  ...
  List.iter render_smoke etat.ref_smoke;
  ...
  List.iter render_smoke etat.ref_explosions;
...
let list_fire = [spawn_fire etat.ref_ship;spawn_fire etat.ref_ship;spawn_fire etat.ref_ship] in
...
let rec boucle_interaction ref_etat =
  if !(!ref_etat.ref_ship).health>0. then (
    ...
    let status = wait_next_event[Key_pressed] in
      ...
      | 's' -> rotation_gauche ref_etat; boucle_interaction ref_etat (* rotation vers la gauche *)
      | 'e' -> acceleration ref_etat;boucle_interaction ref_etat (* acceleration vers l'avant *)
      | 'f' -> rotation_droite ref_etat;boucle_interaction ref_etat (* rotation vers la droite *)
    ...
    game_speed := 0.2;
    ratio_phys_deg := 0.01;
    (*Si la santé du vaisseau est négative -> en cas de mort quoi*)
    (*TODO le code ci-dessous semble quitter instantanément le jeu en cas de mort*)
  (*
    if key_pressed  () then
    let status = wait_next_event[Key_pressed] in
      match status.key  with (* ...en fonction de la touche frappee *)
